...
> **快照日**:2026-05-11(同步 m7_pullon 21-brand + Phase 2 全景接線 + `filterBibleByCategory` 6 維 filter)。
...
flowchart LR
  ...
  subgraph Vercel["Vercel(stytrix-techpack.vercel.app)"]
    direction TB
    SS["Static 託管<br/>index.html · LOGO.png<br/>data/runtime/ (17 檔成品 JSON)<br/>data/source/ · data/legacy/<br/>data/ingest/ (staging,含 m7_pullon/)<br/>pom_rules/ · l2_l3_ie/ · recipes/<br/>path2_universal/<br/>docs/spec/ · docs/sop/ · docs/architecture/"]
  ...
  subgraph Actions["GitHub Actions"]
    A1["rebuild_master.yml<br/>push: data/ingest/uploads/**<br/>     或 data/ingest/m7_pullon/**<br/>(也吃 workflow_dispatch)<br/>觸發全套 Phase 2 重建"]
  ...
  M7 -. "git push entries.jsonl + designs.jsonl.gz<br/>(跨 21 brand,4,562 EIDH)" .-> GH
...
```mermaid
flowchart TB
  Boot["Boot:fetch brands.json<br/>(21 brand,排序 n_designs DESC)"]
  ...
  subgraph Makalot["mode = makalot(聚陽模型)"]
    ...
    M4b["filterBibleByCategory(bible, {brand,<br/>fabric,gender,dept,gt,it})<br/>6 維,lazy fetch designs.jsonl.gz<br/>(native gzip 解壓,cache 4,562 designs)<br/>按 (L2|L3|L4|L5) 重算 sec_median"]
    M5["前端 fetch<br/>pom_rules/&lt;bucket&gt;.json<br/>(81 bucket × gender × dept × gt × fabric)"]
...
Bible 自帶的 `actuals.by_brand[brand]` 只有 1 維(brand)的聚合;要再切性別/品類/紗線等需要回到 per-EIDH 原始工段資料重算 sec_median。`designs.jsonl.gz` 6.4 MB gzipped / ~24 MB JSON parse 後 ~4,562 designs,前端用 native `DecompressionStream('gzip')` 解壓(Chrome/Edge/Firefox + Safari 16.4+ 全支援),parse 一次 cache 在 module scope,後續 filter 切換瞬間完成。失敗 graceful fallback 到 `filterBibleByBrand`。
...
本圖只畫**被前端或 function bundle 消費**的檔;Pipeline 內部用的 `construction_bridge_v6` / `bucket_taxonomy` / `gender_gt_pom_rules` / m7_pullon `entries.jsonl` 雖在 runtime 目錄,線上不讀,見 Section 6 完整目錄盤點。

```mermaid
flowchart TB
  subgraph Top["消費者 (consumers)"]
    direction LR
    ...
    subgraph Fn["api/analyze.js (function bundle, claude-sonnet-4-6)"]
      direction TB
      FN_pass1["identifyL1<br/>(GUIDE + L1_STD + STYLE)"]
      FN_pass2["identifyL2<br/>(GUIDE + TREES + STYLE)"]
    end
  end

  subgraph Runtime["data/runtime/ (前端 / function 消費的部分)"]
    direction LR
    D_brands["brands.json<br/>(Step 4c 產, 21 brand)"]
    D_pomdict["pom_dictionary.json<br/>(兩路共享, admin 可編)"]
  ...
  subgraph DataDirs["其他資料目錄"]
    direction LR
    subgraph IE["l2_l3_ie/ (38 L1 + index, Phase 2 dict)"]
      IE_files["&lt;L1&gt;.json × 38<br/>(actuals.by_brand 21 brand)"]
    end
    subgraph PomRules["pom_rules/ (81 bucket + _index)"]
      P_files["&lt;bucket&gt;.json × 81 + _index"]
    end
    subgraph Path2["path2_universal/"]
      direction TB
      G_v43["iso_lookup_factory_v4.3.json<br/>(primary, 230)"]
      G_v4["iso_lookup_factory_v4.json<br/>(fallback 282)"]
    end
    subgraph Ingest["data/ingest/m7_pullon/"]
      M_designs["designs.jsonl.gz<br/>(per-EIDH 4,562 designs<br/>含 canonical block + 5-level steps)"]
    end
  end

  subgraph Spec["docs/spec/ (includeFiles only)"]
  ...
  D_guide -.-> FN_pass2
  D_trees -.-> FN_pass2
  R_style -. "注入兩個 Pass 的 system prompt" .-> Fn
...
> **兩路共享**:`data/runtime/pom_dictionary.json`(admin 可編)+ `data/runtime/brands.json`(boot eager fetch)。
> **lazy 第二次 fetch**(粗線):`data/ingest/m7_pullon/designs.jsonl.gz` 只在用戶觸發 `filterBibleByCategory` 時才解壓 + parse,parse 完 cache 在 module scope。
> **不在本圖**:`bucket_taxonomy.json`(Pre-3 gate)、`construction_bridge_v6.json`(Step 3 吃)、`gender_gt_pom_rules.json` / `all_designs_gt_it_classification.json` / `client_rules.json` / `design_classification_v5.json`(規則產線輸出,線上目前不讀)。完整 17 檔目錄見 Section 6。

---

## 4. Ingest Pipeline(Phase 2 全景:8 步驟,雙 trigger)

兩條進料路徑:① Admin 上傳 PDF/PPTX(觸發 Step 1-4)、② 聚陽 M7_Pipeline push m7_pullon(只觸發 Step 3-4,因為已是 aggregated jsonl)。兩條都吃同一條 `rebuild_master.yml` workflow,trigger 設 `push: paths: [data/ingest/uploads/**, data/ingest/m7_pullon/**]`。
...
flowchart TB
  ...
  subgraph Triggers["雙進料路徑"]
    ...
    BP3["build_m7_pullon_source_v3.py<br/>→ git push<br/>→ data/ingest/m7_pullon/{entries.jsonl,<br/>designs.jsonl.gz}"]
  end
  ...
  subgraph WF["rebuild_master.yml(8 步,實際順序 1 → 2b → 2a → Pre-3 → 3 → 4a → 4b → 4c)"]
    direction TB
    S1["Step 1 extract_raw_text.py<br/>PDF 首頁元資料 + D-number<br/>PPTX 逐頁文字<br/>callout 頁面評分後 216 DPI → PNG<br/>→ metadata/designs.jsonl (append)<br/>→ pptx/*.txt, pdf/callout_images/*.png<br/>→ pdf/callout_manifest.jsonl<br/>支援 --allow-empty / --force"]
    S2b["Step 2b vlm_pipeline.py --map-iso<br/>(2b 先 2a 後,讓 2a 吃到本 run VLM facts)<br/>若 vlm_raw_extracts.json 不存在 +<br/>ANTHROPIC_API_KEY 有設:<br/>呼 Claude Haiku 分析 callout PNG<br/>→ data/ingest/vlm/{facts.jsonl,<br/>vlm_callout_extracts.json}<br/>continue-on-error: true"]
    ...
    S3["Step 3 build_recipes_master.py --strict<br/>7 來源 cascade 合併:<br/>• recipes/*.json (72 檔)<br/>• consensus_v1/entries.jsonl (275)<br/>• data/ingest/*/facts.jsonl glob<br/>  (vlm/, ocr_v1/, consensus_rules/)<br/>• m7_pullon/entries.jsonl (★ 768 行,21 brand)<br/>• iso_lookup_factory_v4.3.json (primary 230)<br/>• iso_lookup_factory_v4.json (fallback 282)<br/>• construction_bridge_v6.json<br/>aggregation 優先序:<br/>same_sub > same_bucket > same_gt > general > cross_design<br/>→ data/master.jsonl + master.meta.json<br/>→ data/runtime/{iso_dictionary, l1_standard_38}.json<br/>違規 exit 1"]
    ...
    S4b["Step 4b derive_bible_actuals.py --all --in-place<br/>讀 l2_l3_ie/&lt;L1&gt;.json (38 檔,Phase 1 list<br/>或 Phase 2 dict 皆可) + m7_pullon/designs.jsonl.gz<br/>升級 dict schema + 重算 actuals<br/>(2026-05-11 修 idempotency bug,永遠重算)<br/>掛 actuals.by_brand 21 brand<br/>→ 原地寫回 l2_l3_ie/*.json (79 MB)"]
    S4c["Step 4c scripts/core/build_brands.py<br/>從 m7_pullon/entries.jsonl client_distribution<br/>聚合各 brand 的 n_entries / n_designs<br/>排序 n_designs DESC<br/>→ data/runtime/brands.json (~1.8 KB)"]
...
**細節**:
- workflow trigger 是 `push: paths: [data/ingest/uploads/**, data/ingest/m7_pullon/**]` 或手動 `workflow_dispatch`;後者可帶 `force=true` 忽略已處理設計全部重跑。
...
- Step 3 是**全量重建**,不是增量,輸出 deterministic。M7 列管 priority 3 / PDF priority 2 / 推論 priority 1 的 multi-source consensus 在這步合進 `canonical.<field>.{value, confidence, sources}`(來自 m7_pullon source schema,規則表 `data/source/canonical_aliases.json`)。
- **Step 4 是 Phase 2 derive views** — 把 `master.jsonl` 衍生成不同視角:
  - **4a**:`recipes_master.json`(前端通用模型消費的輕量版,剝 `_m7_*` 內部欄)
  - **4b**:`l2_l3_ie/*.json`(升級 Phase 2 dict schema + 掛 m7_pullon actuals,21 brand)
...
| **Step 3 吃的 reference** | `data/runtime/{construction_bridge_v6, bucket_taxonomy, l1_standard_38, pom_dictionary}.json`、`data/ingest/consensus_v1/entries.jsonl`、`path2_universal/iso_lookup_factory_v{4.3,4}.json` |
...
**不送外部**:Step 4b `derive_bible_actuals.py` + Step 4c `build_brands.py` — 需要 `m7_pullon/{entries.jsonl, designs.jsonl.gz}`(聚陽 PullOn 內部工段資料,IP 敏感)+ `l2_l3_ie/*.json` 38 檔(79 MB)。外部跑完 Step 1-4a 後送回 raw `data/ingest/{metadata,unified,vlm}/*.jsonl`,main CI 合併 + 重跑 Step 3 + 4a + 4b + 4c 產最終 view + brands.json。
...
| `data/runtime/brands.json` | 前端 Brand 下拉動態 source(21 brand) | Step 4c 產(2026-05-11 加) |
...
| `data/ingest/m7_pullon/{entries.jsonl, designs.jsonl.gz}` | 聚陽 PullOn pipeline 第 7 個 source(21 brand,4,562 EIDH) | M7_Pipeline push |
...
| `pom_rules/*.json`(81 bucket + `_index`) | 聚陽模型 bucket 規則庫 | 由 `scripts/core/reclassify_and_rebuild.py` 產(線下,Pipeline B) |
| `l2_l3_ie/*.json`(38 L1 + `_index`,Phase 2 dict schema) | 聚陽模型 38 部位 L2-L3-IE 規則 + `actuals.by_brand` 21 brand | **CI 自動產**:Step 4b `derive_bible_actuals.py --all --in-place` |
...
- **`l2_l3_ie/*.json` 79 MB 大檔進 repo** — Step 4b 升級後 38 個 JSON 合計 79.1 MB(從 73.7 MB +7% on 21-brand 升級)。git clone 變慢、PR diff 不友善。考慮:① 拆 brand-actuals 出來放獨立檔(但前端要 N+1 fetch);② 改 lazy-load 只在 L1 點開時 fetch 單檔(現況已是 per-L1 fetch);③ 接受現況(7% 增量還算合理)。**暫不動**。
- **前端大 JSON 無 lazy-load** —
  - `grading_patterns.json` 1.98 MB
  - `gender_gt_pom_rules.json` 1.51 MB
  - `recipes_master.json` 1.39 MB
...
- **m7_pullon 來自聚陽 Windows 端 PowerShell pipeline,沒在本 repo CI** — 重建邏輯不可見,只能信任 push 進來的 entries.jsonl / designs.jsonl.gz schema 正確。schema 驗證靠 Step 3 cascade 中的 bucket_taxonomy gate 攔截。
...
- `iso_lookup_factory_v4.json` 已是 fallback,但仍被前端引用(提供 iso_zh / 機種),不能刪。**這是設計決定,不是 debt**。
...
| `data/source/M7_PULLON_DATA_SCHEMA.md` | `data/source/` | m7_pullon source schema 文件(entries.jsonl / designs.jsonl.gz 欄位語意 + canonical block) | `MK_METADATA.md`、`docs/architecture/PHASE2_DERIVE_VIEWS_SPEC.md` | 2026-05-08 新增 |

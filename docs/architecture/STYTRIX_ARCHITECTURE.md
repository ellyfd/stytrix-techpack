...
>
> **作者**：@elly | **日期**：2026-05-08 v3.0 | **狀態**：implemented v3.2(2026-05-09);Phase 2 View A + B 接線 CI(Step 4a/4b),`l2_l3_ie_by_client/` 已退役;**View C designs_index per-EIDH 也在 2026-05-09 retired**(前端無消費)。實際 view 對應見下方 Step 4 表。
...
- 短暫存在過的 View C = `data/runtime/designs_index/<EIDH>.json` 3,900 檔(2026-05-08 上線給前端 lazy fetch 用)在 **2026-05-09 retired** — 前端無 UI 消費,刪 derive script + workflow Step 4c + dead 產物,設計留 `PHASE2_DERIVE_VIEWS_SPEC.md` 章節作未來重啟參考。

...
| **通用模型** (`universal`) | Path 2：VLM 只判 L1，雙表查 ISO：先 `iso_lookup_factory_v4.2.json`(Department × **Gender** × GT × L1,zone-specific,性別差異)再退 `iso_lookup_factory_v4.json`(Fabric × Department × GT × L1_code,覆蓋完整 + 提供 iso_zh/機種) | 僅 5 項基礎大尺寸（上衣：肩/胸/下擺/袖長/身長；下身：腰/臀/前後檔/內長/褲口） | — |

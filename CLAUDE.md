...
| `data/runtime/*.json` count | **16 檔**(2026-05-14:從 18 縮 3 個 Pipeline B 內部產物搬 `pom_rules/_derive/`,後又新增 `brand_pom_alias.json`)| `ls data/runtime/*.json \| wc -l` |
...
### 待辦 / Roadmap(2026-05-14 架構評估後留下的項目)

下面 D / E / F 是 code review 跑完後,留下來的**已知架構債**。屬於要設計 + 跨多檔改的 task,不在這輪 commit 內;放這裡備忘。
...
| **F** | `bodytype_variance.json` 加 per-brand 維度 | 中,聚陽端寫 `M7_Pipeline/scripts/build_bodytype_variance_per_brand.py` + 改 key schema | **非 ONY brand 的 Petite/Tall delta 變真實值** — 現況整檔 `_meta.source_brand = "ONY"`,key 只有 `<GENDER>\|<GT>\|<BODYTYPE>` 沒 brand 維度;選 DKS/KOH 等其他 brand 看到的 Petite delta 是 ONY 外推,不一定適用。spec 已寫(`docs/architecture/PER_BRAND_BODYTYPE_VARIANCE_SPEC.md`),前端 warning + auto-detect `_meta.per_brand` flag 已 ship(2026-05-14)— 等聚陽端跑 build 推新 schema 即可生效,無需動前端 | 使用者反映「DKS 沒有 Petite & Tall & Plus 嗎? 這必須是品牌專屬」 |
...
|--------|----------------------|------|------------|
| `data/runtime/` | **線上系統 runtime 讀的成品 JSON**(2026-05-14 起 **16 個 .json**,從 18 縮 3 個搬到 `pom_rules/_derive/`、新增 1 個 `brand_pom_alias.json`)。前端 `fetch('./data/runtime/...')`、API `analyze.js` 啟動時讀 | `l1_standard_38.json`、`l2_visual_guide.json`、`l2_decision_trees.json`、`recipes_master.json`、`iso_dictionary.json`、`pom_dictionary.json`、`grading_patterns.json`、`bucket_taxonomy.json`(28 v4 + 59 legacy)、`construction_bridge_v6.json`、`brands.json`(2026-05-11 加,前端 Brand 下拉動態 source)、`code_manifest.json`(2026-05-13 加,index.html「管理 ▸ Code 瀏覽」modal 的檔案清單,由 `scripts/core/build_code_manifest.py` 產生,**不在 CI workflow 內,要手跑**)、`brand_pom_alias.json`(2026-05-14 加,前端 `dataBrandMatches` 用,brands.json 短碼 → pom_rules raw stamps 對照表,由 `scripts/core/build_brand_alias.py` Step 4d 產出)、`all_designs_gt_it_classification.json`(CI Step 2 vlm_pipeline/extract_unified 的 GT backfill fallback,不被前端 fetch)、`l1_iso_recommendations_v1.json`、`l1_part_presence_v1.json`、`bodytype_variance.json` | 原始 xlsx;靜態文件;ingest 中繼檔;Pipeline B 內部產物(放 `pom_rules/_derive/`);**`designs_index/`** 已退役(2026-05-09) |
...
| `path2_universal/` | **通用模型(不分客戶/品牌)的做工推薦資料源**。ISO 工藝代號查表、knit/woven 做工紀錄、PATH2 pipeline 文件。前身為 `General Model_Path2_Construction Suggestion/`(2026-05-07 改名) | `iso_lookup_brandspec_5dim.json`(2026-05-15,完整 5 維 brand-spec ISO 表,多來源聚合,取代舊 `iso_lookup_factory_v4.3.json`+`v4.json` — 那兩本已 git rm)、`iso_lookup_5dim.json`(IE 生產實況 ISO 表)、`PATH2_通用模型_做工推薦Pipeline.md` | 前端 runtime fetch 的檔(那該放 `data/runtime/`) |
| `scripts/core/` | **資料產線腳本**(repo 內部執行的 build / rebuild / extract / search) | `build_l2_visual_guide.py`、`build_bible_skeleton.py`、`build_brands.py`(2026-05-11 加,從 m7 entries.jsonl 聚合產 runtime/brands.json)、`build_code_manifest.py`(2026-05-13 加,掃 curated 目錄產 runtime/code_manifest.json,給 index.html Code 瀏覽 modal)、`build_brand_alias.py`(2026-05-14 加,從 `canonical_aliases.json:客戶` + `client_canonical_mapping.json:[*].{aliases,legacy_subgroup_codes}` 合併產 `data/runtime/brand_pom_alias.json`,CI Step 4d 跑,讓前端 dataBrandMatches 把 brands.json 短碼對到 pom_rules raw truncated stamps)、`run_extract_new.py`、`reclassify_and_rebuild.py`、`enforce_tier1.py` | 共用函式庫(放 `scripts/lib/`);一次性 ad-hoc 腳本 |
...
| `tests/` | **CI smoke test**(2026-05-14 加)。pytest 在 `rebuild_master.yml` Pre-Step 3 + `validate_pom_rules.yml` 都跑(~1s),catch schema drift `validate_buckets.py` 看不到的(`_index.json` parts count、`l2_l3_ie/_index` 38 parts、`brands.json` shape、`recipes/_index` vs disk file 一致性、`brand_pom_alias.json` 涵蓋關鍵 brand 等) | `test_schema.py`(12 個 invariant)、`__init__.py` | 業務邏輯 unit test(scope 限 schema invariant);跨多檔的整合測試 |
...
| ~~`path2_universal/iso_lookup_factory_v4.json` + `v4.3.json`~~ | 2026-05-15 已 git rm — `build_recipes_master.py` 的 `build_from_v43`+`build_from_v4` 兩段已換成單一 `build_from_brandspec_5dim`(吃 `iso_lookup_brandspec_5dim.json` 完整 5 維表,roll 成 same_gt+general 兩層);`build_l1_standard_38` 也移除 v4.3 fallback(改純讀 `data/runtime/l1_standard_38.json` SOT);index.html / analyze.js 引用同步 re-point。grep 確認全 repo 無 code 相依 | **已清** |

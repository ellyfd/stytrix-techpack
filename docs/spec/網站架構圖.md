...
flowchart LR
  ...
  subgraph Fn["api/analyze.js(function bundle, claude-sonnet-4-6)"]
    FN_pass1["identifyL1(GUIDE + L1_STD + STYLE)"]
    FN_pass2["identifyL2(GUIDE + TREES + STYLE)"]
  end

  subgraph Runtime["data/runtime/(17 檔成品 JSON,線上 fetch / includeFiles)"]
    D_brands["brands.json<br/>(21 brand,Step 4c 產)"]
    ...
    D_recipes["recipes_master.json<br/>(Step 4a derive 後產)"]
    D_isodict["iso_dictionary.json<br/>(Step 3 產)"]
    D_l1std["l1_standard_38.json<br/>(Step 3 產)"]
    D_bridge["construction_bridge_v6.json"]
    D_taxonomy["bucket_taxonomy.json<br/>(28 v4 + 59 legacy,Pre-3 gate)"]
    D_gender["gender_gt_pom_rules.json<br/>all_designs_gt_it_classification.json<br/>client_rules.json / design_classification_v5.json"]
    D_pres["l1_part_presence_v1.json"]
    D_iso["l1_iso_recommendations_v1.json"]
  end

  subgraph Ingest["data/ingest/m7_pullon/(聚陽 PullOn source)"]
    M_entries["entries.jsonl<br/>(aggregated by 6-dim key,768 行)"]
  ...
  subgraph IE["l2_l3_ie/(38 個 L1 + index,Phase 2 dict schema)"]
  ...
  subgraph Path2["path2_universal/"]
    ...
    G_v4["iso_lookup_factory_v4.json<br/>(fallback 282, iso_zh / 機種)"]
  end
...
> **lazy 第二次 fetch**:`data/ingest/m7_pullon/designs.jsonl.gz` 只在用戶觸發 `filterBibleByCategory` 時才解壓 + parse,parse 完 cache 在 module scope。
> `data/runtime/gender_gt_pom_rules.json`(1.5 MB)/ `all_designs_gt_it_classification.json`(1292 designs)雖在 runtime 目錄,目前線上不讀,是規則產線輸出物。

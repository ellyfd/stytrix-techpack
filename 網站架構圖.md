...
flowchart TB
  ...
  subgraph WF["GitHub Actions：rebuild_master.yml<br/>trigger: push paths data/ingest/uploads/**<br/>(workflow_dispatch 也支援，可 force 重跑)"]
    ...
    S2a["Step 2a extract_unified.py<br/>全量合併 4 來源 (PPTX 中文 / PDF 英文 / cb / dir5)<br/>PPTX JSON > PPTX txt > 其他<br/>→ data/ingest/unified/{dim,facts}.jsonl"]
    S2b["Step 2b vlm_pipeline.py --map-iso<br/>讀 vlm_raw_extracts.json(手填或 API 預填)<br/>→ 200+ ISO 術語 + 30+ zone 對映<br/>→ data/ingest/<b>vlm/</b>{facts.jsonl, vlm_callout_extracts.json}<br/>需 ANTHROPIC_API_KEY;continue-on-error"]
    S3["Step 3 build_recipes_master.py --strict<br/>5 來源合併:<br/>• iso_lookup_factory_v4.3.json (primary)<br/>• iso_lookup_factory_v4.json (fallback)<br/>• data/construction_bridge_v6.json<br/>• recipes/*.json (71 檔)<br/>• data/ingest/consensus_v1/entries.jsonl (275 條)<br/>+ data/ingest/*/facts.jsonl<br/>→ data/recipes_master.json<br/>→ data/iso_dictionary.json<br/>→ data/l1_standard_38.json<br/>違規 exit 1"]
    S4["git rm data/ingest/uploads/*<br/>+ git add 重建結果<br/>commit msg: 'chore(data): auto-rebuild recipes_master [skip ci]'"]
    S1 --> S2a --> S2b --> S3 --> S4
  end
...
- Step 2b 實際輸出路徑是 `data/ingest/vlm/`（不是 `vlm_v1/`，是之前的筆誤）；失敗不擋 build。
...
- **前端 CI 追蹤**：`UploadModal` 拿 sha 後 poll `GET /repos/.../actions/runs?head_sha=<sha>`，依序顯示 5 個 step：`1｜拆解文字 & callout 圖片` → `2a｜統一萃取 (PPTX)` → `2b｜VLM 分析 callout (PDF)` → `3｜重建 recipes_master` → `提交資料`。  

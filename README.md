...
>
> 變動 changelog 已搬到 [`CLAUDE.md` Part A 快照區](./CLAUDE.md#part-a--資料夾分工表)。
...
2. **GitHub Actions 觸發**(`.github/workflows/rebuild_master.yml`,`push: paths: data/ingest/uploads/**` 或 `data/ingest/m7_pullon/**`)。實際步驟順序是 **1 → 2b → 2a → Pre-3 → 3 → 4a → 4b → 4c**(2b 刻意排在 2a 前,讓 2a 的 unified 合併能吃到本次 run 剛產出的 VLM facts;Pre-3 跑 `validate_buckets.py --strict` 早期 schema gate;Step 4 是 derive views,把 master.jsonl + m7_pullon designs.jsonl.gz 衍生成 runtime view A/B + brands.json):
   ...
   - **Step 4b** `derive_bible_actuals.py --all --in-place` — Phase 2 View B:讀 `l2_l3_ie/<L1>.json`(IE-xlsx 來的 Phase 1 5-elem list 或上一輪產的 Phase 2 dict 皆可,2026-05-11 修掉 dict pass-through bug,**永遠重算 actuals**)+ `data/ingest/m7_pullon/designs.jsonl.gz`(4,562 件實做工段,跨 21 brand),原地升級成 dict schema `{l5, ie_standard:{sec,grade,primary,machine}, actuals?:{n_steps,n_designs,sec_median,sec_p25,sec_p75,by_brand,machine_top}}`,把 m7_pullon 實際工段數據掛在對應 step 的 `actuals` 欄。
   - **Step 4c** `scripts/core/build_brands.py` — 從 `data/ingest/m7_pullon/entries.jsonl` 的 `client_distribution` 聚合每個 brand 的 n_entries / n_designs,排序 n_designs DESC 寫進 `data/runtime/brands.json`(~1.8 KB)。前端 boot 時 eager fetch 這個檔當 Brand 下拉 source(2026-05-11+ 不再硬寫 BRANDS 常數);新 brand 進 m7_pullon → CI 重產 brands.json → 用戶 reload 就看到。

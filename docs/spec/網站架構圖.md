...
flowchart LR
  ...
  subgraph Vercel["Vercel(stytrix-techpack.vercel.app)"]
    direction TB
    SS["Static 託管<br/>index.html · LOGO.png<br/>data/runtime/ (17 檔成品 JSON)<br/>data/source/ · data/legacy/<br/>data/ingest/ (staging,含 m7/)<br/>pom_rules/ · l2_l3_ie/ · recipes/<br/>path2_universal/<br/>docs/spec/ · docs/sop/ · docs/architecture/"]
...
- **不在本圖**(Pipeline 內部用,線上不讀):`bucket_taxonomy.json`(Pre-3 gate)、`construction_bridge_v6.json`(Step 3 吃)、`gender_gt_pom_rules.json` / `all_designs_gt_it_classification.json` / `client_rules.json` / `design_classification_v5.json`(規則產線輸出)、m7 `entries.jsonl`(Step 3 cascade 吃)。完整 17 檔目錄盤點見 Section 6。
...
flowchart TB
  ...
  subgraph WF["rebuild_master.yml(8 步,實際順序 1 → 2b → 2a → Pre-3 → 3 → 4a → 4b → 4c)"]
    ...
    S3["Step 3 build_recipes_master.py --strict<br/>7 來源 cascade 合併:<br/>• recipes/*.json (72 檔)<br/>• consensus_v1/entries.jsonl (275)<br/>• data/ingest/*/facts.jsonl glob<br/>  (vlm/, ocr_v1/, consensus_rules/)<br/>• m7/entries.jsonl (★ 768 行,21 brand)<br/>• iso_lookup_factory_v4.3.json (primary 230)<br/>• iso_lookup_factory_v4.json (fallback 282)<br/>• construction_bridge_v6.json<br/>aggregation 優先序:<br/>same_sub > same_bucket > same_gt > general > cross_design<br/>→ data/master.jsonl + master.meta.json<br/>→ data/runtime/{iso_dictionary, l1_standard_38}.json<br/>違規 exit 1"]
    ...
    S4b["Step 4b derive_bible_actuals.py --all --in-place<br/>讀 l2_l3_ie/&lt;L1&gt;.json (38 檔,Phase 1 list<br/>或 Phase 2 dict 皆可) + m7/designs.jsonl.gz<br/>升級 dict schema + 重算 actuals<br/>(2026-05-11 修 idempotency bug,永遠重算)<br/>掛 actuals.by_brand 21 brand<br/>→ 原地寫回 l2_l3_ie/*.json (79 MB)"]
...
- **Step 4 是 Phase 2 derive views** — 把 `master.jsonl` 衍生成不同視角:
  ...
  - **4b**:`l2_l3_ie/*.json`(升級 Phase 2 dict schema + 掛 m7 actuals,21 brand)
...
**不送外部**:Step 4b `derive_bible_actuals.py` + Step 4c `build_brands.py` — 需要 `m7/{entries.jsonl, designs.jsonl.gz}`(聚陽 PullOn 內部工段資料,IP 敏感)+ `l2_l3_ie/*.json` 38 檔(79 MB)。外部跑完 Step 1-4a 後送回 raw `data/ingest/{metadata,unified,vlm}/*.jsonl`,main CI 合併 + 重跑 Step 3 + 4a + 4b + 4c 產最終 view + brands.json。
...
| `data/ingest/m7/{entries.jsonl, designs.jsonl.gz}` | 聚陽 PullOn pipeline 第 7 個 source(21 brand,4,562 EIDH) | M7_Pipeline push |

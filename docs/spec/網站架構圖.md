...
flowchart LR
  ...
  subgraph Vercel["Vercel(stytrix-techpack.vercel.app)"]
    direction TB
    SS["Static 託管<br/>index.html · LOGO.png<br/>data/runtime/ (18 檔成品 JSON)<br/>data/source/ · data/legacy/<br/>data/ingest/ (staging,含 m7/)<br/>pom_rules/ · l2_l3_ie/ · recipes/<br/>path2_universal/<br/>docs/spec/ · docs/sop/ · docs/architecture/"]
...
- **不在本圖**(Pipeline 內部用,線上不讀):`bucket_taxonomy.json`(Pre-3 gate)、`construction_bridge_v6.json`(Step 3 吃)、`gender_gt_pom_rules.json` / `all_designs_gt_it_classification.json` / `client_rules.json` / `design_classification_v5.json`(規則產線輸出)、m7 `entries.jsonl`(Step 3 cascade 吃)。完整 18 檔目錄盤點見 Section 6。
...
flowchart TB
  ...
  subgraph WF["rebuild_master.yml(8 步,實際順序 1 → 2b → 2a → Pre-3 → 3 → 4a → 4b → 4c)"]
    ...
    S3["Step 3 build_recipes_master.py --strict<br/>7 來源 cascade 合併:<br/>• recipes/*.json (72 檔)<br/>• consensus_v1/entries.jsonl (275)<br/>• data/ingest/*/facts.jsonl glob<br/>  (vlm/, ocr_v1/, consensus_rules/)<br/>• m7/entries.jsonl (★ 5,076 行,24 brand)<br/>• iso_lookup_factory_v4.3.json (primary 230)<br/>• iso_lookup_factory_v4.json (fallback 282)<br/>• construction_bridge_v6.json<br/>aggregation 優先序:<br/>same_sub > same_bucket > same_gt > general > cross_design<br/>→ data/master.jsonl + master.meta.json<br/>→ data/runtime/{iso_dictionary, l1_standard_38}.json<br/>違規 exit 1"]

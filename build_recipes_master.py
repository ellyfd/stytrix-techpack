...
STAR_SCHEMA = ROOT.parent / "star_schema"  # sibling: ONY/star_schema/
...
def build_zh_to_l1(l1_std: dict) -> dict:
    ...
    for code, info in (l1_std.get("codes") or {}).items():
        ...
        if zh:
            out[zh] = code
    # Aliases: PPTX translations use variant names for the same L1 code
    ZH_ALIASES = {
        "脇邊": "SR",      # 脅邊 variant character (脇=脅)
        "袖孔": "AH",      # 袖窿 alias
        "帽子": "HD",      # 帽 alias
        "前襟": "PL",      # 門襟 alias
        "前拉鍊": "ZP",    # 拉鏈 alias
        "褲口": "BM",      # 下襬 for bottoms
        "褲合身": "RS",    # 起翹 / crotch seam
        "滾邊": "AE",      # 裝飾邊 / binding
        "腰線": "WB",      # 腰頭 alias
        "繩類": "TH",      # 腰繩 alias
        "車縫(通則)": "ST", # 通用車線
        "剪接線": "SR",    # 接合線 → side seam
        "剪接線_上身類": "SR",  # 上身接合線
        "剪接線_下身類": "SR",  # 下身接合線
        "褲襠": "RS",      # 股下/crotch → rise
        "袖圍": "SL",      # 袖（圍度）→ sleeve
    }
...
def main():
    ...
    # same_bucket layer from consensus (if available)
    consensus_entries = []
    consensus_stats = {"loaded": 0, "no_taxonomy": 0}
    if CONSENSUS_PATH.exists() and BUCKET_TAX_PATH.exists():
    ...
    # Cascade order: same_sub → same_bucket → same_gt → general → cross_design
    all_entries = recipe_entries + consensus_entries + v43_entries + v4_entries + bridge_entries
    ...
    out = {
        ...
        "stats": {
            ...
            "same_bucket": len(consensus_entries),

...
| **通用模型** (`universal`) | Path 2：VLM 只判 L1，查 `data/runtime/recipes_master.json` 的 cascade（same_sub → same_bucket → same_gt → general → cross_design）。其中 same_gt / general 兩層由單一完整 5 維表 `path2_universal/iso_lookup_brandspec_5dim.json`（Fabric × Department × Gender × GT × L1，多來源聚合）在 build 時 roll 出（2026-05-15 取代舊 `iso_lookup_factory_v4.3` + `v4` 兩本部分維度表） | 僅 5 項基礎大尺寸（上衣：肩/胸/下擺/袖長/身長；下身：腰/臀/前後檔/內長/褲口） | — |
...
data/
  ...
  └─ ingest/                                  ← PDF/PPTX 上傳後的 staging 區,Actions workflow 讀寫
      ...
      ├─ pdf/construction_images/<DID>_p<N>.png ← PDF construction 頁影像(Step 1,216 DPI,處理完不保留;2026-05-12 rename from `callout_images/`)
      ├─ pdf/construction_manifest.jsonl      ← construction 頁影像索引
      ├─ unified/{dim,facts}.jsonl            ←   Step 2a 全量合併 4 來源的 facts
      ├─ vlm/{facts.jsonl,vlm_construction_extracts.json}
...
path2_universal/                              ← 通用模型(不分客戶/品牌)的做工推薦資料源(原 General Model_Path2_Construction Suggestion/,2026-05-07 改名)
  ├─ iso_lookup_brandspec_5dim.json           ← **brand-spec ISO 表**:完整 Fabric × Department × Gender × GT × L1,多來源聚合(2026-05-15 取代 iso_lookup_factory_v4.3 + v4 兩本)
  ├─ iso_lookup_5dim.json                     ← **IE 生產實況 ISO 表**:同 5 維 key,由聚陽五階層 machine 反查(另一鏡頭,勿與 brandspec 混)
...
star_schema/scripts/                          ← **線上 ingest pipeline**(由 .github/workflows/rebuild_master.yml 呼叫)
  ├─ extract_raw_text.py                      ← Step 1:掃 uploads/,抽 PPTX 文字 + PDF construction 頁圖;支援 --allow-empty / --force
...
| `star_schema/scripts/` | `extract_raw_text.py` | `data/ingest/uploads/*.pdf`、`*.pptx` | `data/ingest/metadata/designs.jsonl` + `pptx/*.txt` + `pdf/construction_images/*.png` | Pipeline A(recipes_master)Step 1 | **活** — CI 每次 push uploads/ 自動跑 |
...
- 重建 **recipes_master**(做工配方)→ 用 `star_schema/scripts/extract_*.py`。不關心 MC/POM 尺寸,抓的是 construction 頁(舊稱 callout)+ ISO zone
...
2. **GitHub Actions 觸發**(`.github/workflows/rebuild_master.yml`,`push: paths: data/ingest/uploads/**` 或 `data/ingest/m7/**`)。實際步驟順序是 **1 → 2b → 2a → Pre-3 → 3 → 4a → 4b → 4c**(2b 刻意排在 2a 前,讓 2a 的 unified 合併能吃到本次 run 剛產出的 VLM facts;Pre-3 跑 `validate_buckets.py --strict` 早期 schema gate;Step 4 是 derive views,把 master.jsonl + m7 designs.jsonl.gz 衍生成 runtime view A/B + brands.json):
   - **Step 1** `extract_raw_text.py` — 掃 uploads/；PDF 讀首頁元資料(季節 / 品牌 / 類型)+ 抽 D-number，PPTX 逐頁取文字，PDF construction 頁面評分後 216 DPI 渲染成 PNG(2026-05-12 rename:`callout` → `construction`)。`--allow-empty` 允許首次空掃，`--force` 忽略已處理重跑。
   - **Step 2b** `vlm_pipeline.py --map-iso --ingest-dir data/ingest --out data/ingest/vlm` — 若沒預填 `vlm_raw_extracts.json` 但 `ANTHROPIC_API_KEY` 有設，會對每張 construction PNG 呼叫 Claude Haiku 自動分析（`requirements-pipeline.txt` 加 `anthropic>=0.25.0`）；否則讀既有 `vlm_raw_extracts.json` 走手填路徑。輸出 `vlm/facts.jsonl`（含 `bucket` 欄位，Step 3 才能用）+ `vlm_construction_extracts.json`。**資料夾是 `vlm/`，不是 `vlm_v1/`**。`continue-on-error`，失敗不擋 build。
   ...
   - **Step 3** `build_recipes_master.py --strict` — 合併 7 份來源 → 全量重建:
     ...
     - `path2_universal/iso_lookup_brandspec_5dim.json`(完整 5 維 brand-spec ISO 表,build 時 roll 成 same_gt + general 兩層;2026-05-15 取代舊 iso_lookup_factory_v4.3 + v4)

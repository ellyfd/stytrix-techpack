...
> 3. **bucket_taxonomy v4 + legacy_buckets**:`data/bucket_taxonomy.json` 改成 v4 4-dim key (`<gender>_<dept>_<garment>_<item>`,JSON 內部 key 簡寫為 `gt`/`it`) 28 個 + `legacy_buckets` 59 個 alias 給舊 facts/consensus 用。

...
> - **Phase 2 derive view spec** 已寫(`docs/architecture/PHASE2_DERIVE_VIEWS_SPEC.md`):master.jsonl 拆三 view (recipes_master / l2_l3_ie 升級 schema / designs_index per-EIDH)。**View A + View B 已接線**(CI workflow Step 4a/4b 跑 `derive_view_recipes_master.py` + `derive_view_l2_l3_ie.py --all --in-place`,`l2_l3_ie/<L1>.json` 38 檔已升級為 dict schema `{l5, ie_standard, actuals?}`);View C designs_index per-EIDH 仍 TBD。
> - **bucket_taxonomy v4**:`data/runtime/bucket_taxonomy.json` 28 個 4-dim bucket,schema 已 fully v4-aligned 不需 legacy fan-out(歷史 `legacy_buckets` 已不再寫入,`build_recipes_master.py` 仍 tolerate `.get("legacy_buckets") or {}`)。
>
...
| [`docs/architecture/PHASE2_DERIVE_VIEWS_SPEC.md`](./docs/architecture/PHASE2_DERIVE_VIEWS_SPEC.md) | Phase 2 三個 derive view 設計:master.jsonl → recipes_master(view A,輕量化)/ l2_l3_ie 升級 schema(view B,drop brand,含 actuals)/ designs_index per-EIDH(view C,含 brand × knit/woven)。**View A + B 已接線**(CI Step 4a/4b);View C designs_index 仍 TBD |
...
2. **GitHub Actions 觸發**（`.github/workflows/rebuild_master.yml`，`push: paths: data/ingest/uploads/**`）。實際步驟順序是 **1 → 2b → 2a → 3 → 4a → 4b**（2b 刻意排在 2a 前,讓 2a 的 unified 合併能吃到本次 run 剛產出的 VLM facts;Step 4 是 Phase 2 derive views,把 master.jsonl 衍生成 runtime view A/B):

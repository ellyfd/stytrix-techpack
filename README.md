...
>
> **更新 2026-05-09**:
> - **Phase 2 derive views 全景完成** — View A(recipes_master 輕量化)/ View B(`l2_l3_ie/*.json` 38 檔升級為 Phase 2 dict schema `{l5, ie_standard, actuals?}` + 掛 m7_pullon 觀察值)/ View C(per-EIDH `data/runtime/designs_index/` 讓前端 lazy fetch 單一 design 完整履歷)三個 view 都已接線 CI(Step 4a/4b/4c)。spec 見 [`docs/architecture/PHASE2_DERIVE_VIEWS_SPEC.md`](./docs/architecture/PHASE2_DERIVE_VIEWS_SPEC.md)。
> - **CI workflow step**:`1 → 2b → 2a → Pre-3(validate buckets)→ 3 → 4a → 4b → 4c → commit`。Pre-3 跑 `scripts/core/validate_buckets.py --strict` 早期 schema gate(<1s,catch drift)。
...
| [`docs/architecture/PHASE2_DERIVE_VIEWS_SPEC.md`](./docs/architecture/PHASE2_DERIVE_VIEWS_SPEC.md) | Phase 2 三個 derive view 設計:master.jsonl → recipes_master(view A,輕量化)/ l2_l3_ie 升級 schema(view B,drop brand,含 actuals)/ designs_index per-EIDH(view C,含 brand × knit/woven)。**View A + B + C 全已接線**(CI Step 4a/4b/4c) |
...
2. **GitHub Actions 觸發**(`.github/workflows/rebuild_master.yml`,`push: paths: data/ingest/uploads/**`)。實際步驟順序是 **1 → 2b → 2a → Pre-3 → 3 → 4a → 4b → 4c**(2b 刻意排在 2a 前,讓 2a 的 unified 合併能吃到本次 run 剛產出的 VLM facts;Pre-3 跑 `validate_buckets.py --strict` 早期 schema gate;Step 4 是 Phase 2 derive views,把 master.jsonl + m7_pullon designs.jsonl.gz 衍生成 runtime view A/B/C):
   ...
   - **Step 4a** `derive_view_recipes_master.py` — Phase 2 View A:讀 `data/master.jsonl`,把每筆 entry 的 `_m7_*` 內部欄位剝掉(它們是給 View B/C 用的,前端不消費,留著會讓 `recipes_master.json` 多 ~15x),覆寫 `data/runtime/recipes_master.json`。
   ...
   - **Step 4c** `derive_view_designs_index.py` — Phase 2 View C:把 `data/ingest/m7_pullon/designs.jsonl.gz` 拆成 per-EIDH `data/runtime/designs_index/<EIDH>.json` 3,900 個小檔,讓前端 lazy fetch 單一 design 完整履歷(含 5-level 工段、客戶 metadata、canonical block)而不用整包載 6.6 MB gzip。
...
📦 下載後交給協作單位,內含:
- **Pipeline A 腳本**:`star_schema/scripts/*.py`(7 檔總共,外送包 bundle 5 檔 — Step 1-3 + Step 4a `derive_view_recipes_master.py`;**Step 4b `derive_view_l2_l3_ie.py` + 4c `derive_view_designs_index.py` 不送外部** — 都需要 `data/ingest/m7_pullon/designs.jsonl.gz`(聚陽 PullOn 內部工段資料,IP 敏感),且 4b 還要 `l2_l3_ie/*.json` 38 檔(74 MB),由 main CI 接手。外部跑完 Step 1-4a 後送回 raw `data/ingest/{metadata,unified,vlm}/*.jsonl`,main CI 合併 + 重跑 Step 3 + 4a + 4b + 4c 產最終 3 個 view)

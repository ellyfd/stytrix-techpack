...
- `scripts/`（線下規則產線 10 檔）與 `star_schema/scripts/`（線上 ingest pipeline 4 檔）是**兩組獨立**的 pipeline，彼此不共用程式碼，也沒有統一入口；動其中一邊不會自動影響另一邊。**合併要大重構**。
- CI polling 的 graceful fallback 設計良好，但當 Actions API 完全無法存取時，使用者只會看到「點連結去 GitHub」提示，無法在 app 內看到實際失敗原因。**可改但要設計錯誤顯示 UX**。

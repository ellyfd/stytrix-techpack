...
| **通用模型** (`universal`) | Path 2：VLM 只判 L1，雙表查 ISO：先 `iso_lookup_factory_v4.3.json`(Dept × **Gender** × GT × L1，fine GT 對齊 UI 下拉選單，230 entries)再退 `iso_lookup_factory_v4.json`(Fabric × Dept × GT × L1_code，282 entries，提供 iso_zh/機種)；另吃 `recipes_master.json` 顯示 recipe 推薦 | 僅 5 項基礎大尺寸（上衣：肩/胸/下擺/袖長/身長；下身：腰/臀/前後檔/內長/褲口） | — |

...
# ═══════════════════════════════════════════════
# ② grading_patterns  (3D: Dept_GT|Gender, aligned with pom_rules)
# ═══════════════════════════════════════════════
print("\n=== ② Building grading_patterns (3D) ===")

MIN_GRADING_N = 3  # minimum designs to compute grading

# ── Group designs by 3D key: Dept_GT|Gender (matches pom_rules bucket format) ──
dept_gt_gender_groups = defaultdict(list)  # "RTW_TOP|WOMENS" -> [design_ids]
for did, cls in designs_cls.items():
    if did in profiles_by_id:
        key_3d = f"{cls['dept']}_{cls['gt']}|{cls['gender']}"
        dept_gt_gender_groups[key_3d].append(did)

# Also keep 2D fallback groups (Gender|GT) for low-sample combos
# gender_gt_groups already built above in ①
...
def grading_result_from_pom_gradings(pom_gradings):
    """Convert raw pom_gradings into final result dict with inflection detection."""
    combo_result = {}
...
# ── Build grading with 3D key + fallback ──
grading = {}
fallback_used = []
direct_count = 0

# Collect all pom_rules bucket keys for reference
pom_rules_idx_path = os.path.join(BASE, 'pom_rules', '_index.json')
pom_rules_buckets = set()
if os.path.exists(pom_rules_idx_path):
    with open(pom_rules_idx_path) as f:
...
for key_3d, dids in sorted(dept_gt_gender_groups.items()):
    ...
    pom_gradings = compute_grading_for_dids(dids, gender)

    if len(pom_gradings) >= MIN_GRADING_N:
        result = grading_result_from_pom_gradings(pom_gradings)
        if result:
            grading[key_3d] = result
            grading[key_3d]['_meta'] = {'n_designs': len(dids), 'source': 'direct'}
            direct_count += 1
    else:
        # Fallback to Gender|GT (2D) — use the larger pool
        dept_gt = key_3d.split('|')[0]
        gt = dept_gt.split('_', 1)[1] if '_' in dept_gt else dept_gt
        fallback_key = f"{gender}|{gt}"
        fb_dids = gender_gt_groups.get(fallback_key, [])
        if fb_dids:
            fb_gradings = compute_grading_for_dids(fb_dids, gender)
            if len(fb_gradings) >= MIN_GRADING_N:
                result = grading_result_from_pom_gradings(fb_gradings)
                if result:
                    grading[key_3d] = result
                    grading[key_3d]['_meta'] = {
                        'n_designs': len(dids),
                        'source': 'fallback',
                        'fallback_key': fallback_key,
                        'fallback_n': len(fb_dids)
                    }
                    fallback_used.append((key_3d, fallback_key, len(dids), len(fb_dids)))

# ── Also emit 2D keys for any Gender|GT combos NOT covered by 3D ──
# This ensures backward compatibility if frontend hasn't switched to 3D keys yet
grading_2d_compat = {}
for combo_2d, dids in sorted(gender_gt_groups.items()):
    ...
    if len(pom_gradings) >= MIN_GRADING_N:
        result = grading_result_from_pom_gradings(pom_gradings)
        if result:
            grading_2d_compat[combo_2d] = result
            grading_2d_compat[combo_2d]['_meta'] = {'n_designs': len(dids), 'source': '2d_compat'}

# Merge: 3D keys take priority, 2D added for backward compat
for k2d, v2d in grading_2d_compat.items():
    if k2d not in grading:
        grading[k2d] = v2d

# Compute overall inflection rate
total_pom_families = sum(len({k for k in v if k != '_meta'}) for v in grading.values())
inflection_count = sum(1 for v in grading.values() for k, p in v.items() if k != '_meta' and p.get('inflection'))
inflection_rate = round(inflection_count / total_pom_families * 100, 1) if total_pom_families > 0 else 0

# Count how many pom_rules buckets now have grading
...
print(f"  3D direct: {direct_count}, fallback: {len(fallback_used)}, 2D compat: {len(grading_2d_compat)}")
print(f"  Total combos: {len(grading)}, {total_pom_families} POM families, inflection {inflection_rate}%")
print(f"  pom_rules coverage: {covered}/{len(pom_rules_buckets)} buckets have grading data")
if fallback_used:
    print(f"  Fallback details:")
    for k3d, fb, n3d, nfb in fallback_used[:10]:
